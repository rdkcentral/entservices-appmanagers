...
public:
    ...
    using ListenerPropertyMap = std::map<std::string, RdkWindowManagerData>;
    using EventPayload = std::vector<std::map<std::string, RdkWindowManagerData>>;
    ...
    MOCK_METHOD(
        ...
        (const std::string& client,
         ...
         ListenerPropertyMap& listenerProperties),
        (override));
    MOCK_METHOD(
        bool,
    ...
    MOCK_METHOD(
        bool,
        generateKey,
        (const std::string& client, const uint32_t& keyCode, const uint32_t& flags, std::string virtualKey),
        (override));
    MOCK_METHOD(
        bool,
        generateKey,
        (const std::string& client, const uint32_t& keyCode, const uint32_t& flags, std::string virtualKey, double duration),
        (override));
    ...
    MOCK_METHOD(bool, sendEvent, (const std::string& eventName, EventPayload& data), (override));
