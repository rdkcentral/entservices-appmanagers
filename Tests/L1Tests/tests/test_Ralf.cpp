...
/* Test Case: AddToEnvironment_DuplicateKeyReplaced
 * Verifies that when the same key is added twice, the old value is removed
 * and replaced with the new value (last-write-wins, deduplication).
 * This is critical for the O(N) deduplication optimization.
 */
TEST_F(RalfOCIConfigGeneratorPrivateTest, AddToEnvironment_DuplicateKeyReplaced)
{
    TEST_LOG("Testing addToEnvironment with duplicate key (deduplication)");
    Json::Value root;
    mAcc.addToEnvironment(root, "MYVAR", "old_value");
    mAcc.addToEnvironment(root, "MYVAR", "new_value");
    const Json::Value &envArr = root[ralf::PROCESS][ralf::ENV];
    ASSERT_EQ(1u, envArr.size());  // Should have exactly 1 entry (not 2)
    EXPECT_EQ("MYVAR=new_value", envArr[0].asString());
}

/* Test Case: AddToEnvironment_MultiплеWithDuplicate
 * Verifies that deduplication works correctly when mixed with other variables:
 * adding var1, var2, var1 again should result in [var2, var1] order.
 */
TEST_F(RalfOCIConfigGeneratorPrivateTest, AddToEnvironment_MultipleWithDuplicate)
{
    TEST_LOG("Testing addToEnvironment with multiple calls including duplicate");
    Json::Value root;
    mAcc.addToEnvironment(root, "VAR1", "first");
    mAcc.addToEnvironment(root, "VAR2", "second");
    mAcc.addToEnvironment(root, "VAR1", "updated");  // VAR1 should be moved to end
    const Json::Value &envArr = root[ralf::PROCESS][ralf::ENV];
    ASSERT_EQ(2u, envArr.size());
    EXPECT_EQ("VAR2=second", envArr[0].asString());
    EXPECT_EQ("VAR1=updated", envArr[1].asString());
}
