...
TEST_F(PackageManagerTest, getConfigForPackageusingComRpcLibPackageMockSuccess) {
    ...
    EXPECT_CALL(*libPackageMock, GetFileMetadata(fileLocator, ::testing::_, ::testing::_, ::testing::_))
        .WillOnce(::testing::Invoke([](const std::string&, std::string& id, std::string& ver, packagemanager::ConfigMetaData& metadata) {
            id = "YouTube";
            ver = "100.1.24";
            metadata.dataImageSize = 2048;
            return packagemanager::SUCCESS;
        }));

    EXPECT_EQ(Core::ERROR_NONE, pkginstallerInterface->GetConfigForPackage(fileLocator, packageId, version, config));
    EXPECT_EQ("YouTube", packageId);
    EXPECT_EQ("100.1.24", version);
    EXPECT_EQ(2048u, config.dataImageSize);

    deinitforComRpc();
    packagemanager::IPackageImplDummy::ResetMockInstance();
}

TEST_F(PackageManagerTest, getConfigForPackageusingComRpcLibPackageMockFailure) {
    ...
    EXPECT_CALL(*libPackageMock, GetFileMetadata(fileLocator, ::testing::_, ::testing::_, ::testing::_))
        .WillOnce(::testing::Return(packagemanager::FAILED));
...
TEST_F(PackageManagerTest, installusingComRpcLibPackageMockVersionMismatch) {
