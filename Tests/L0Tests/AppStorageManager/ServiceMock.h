...
public:
    ...
    void SetConfigLine(const std::string& config)
    {
        _configLine = config;
    }

    void SetPersistentStore(WPEFramework::Exchange::IStore2* store)
    {
        if (_persistentStore) {
            _persistentStore->Release();
        }
    ...
    std::string Callsign() const override { return "org.rdk.AppStorageManager"; }
    ...
    void* QueryInterfaceByCallsign(const uint32_t id, const std::string& callsign) override
    {
        if (WPEFramework::Exchange::IStore2::ID == id
            && "org.rdk.PersistentStore" == callsign
            && nullptr != _persistentStore)
        {
            _persistentStore->AddRef();
            return static_cast<WPEFramework::Exchange::IStore2*>(_persistentStore);
        }
        return nullptr;
    }
...
private:
    ...
    WPEFramework::Exchange::IStore2* _persistentStore;
