...
bool AppManagerImplementation::UpdateAppInfo(const std::string& appId,
                                             const AppInfo& updatedInfo)
{
    mAdminLock.Lock();
    auto it = mAppInfo.find(appId);
    if (it == mAppInfo.end()) {
        LOGWARN("UpdateAppInfo: App %s not found", appId.c_str());
        return false;
    }

    it->second = updatedInfo;  //  overwrite existing entry safely
    mAdminLock.Unlock();
    LOGINFO("Updated appData for appId %s", appId.c_str());
    return true;
}
